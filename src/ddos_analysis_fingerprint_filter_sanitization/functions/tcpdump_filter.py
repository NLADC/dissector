...
def tcpdump_filter(input_file, pattern):
    ...
    if len(pattern['src_ips']) > 0:
        src_ip_filter = ["src host " + ip for ip in pattern['src_ips']]
...
#     f.write(filter_out)
#     f.close()
    p = subprocess.Popen(["tcpdump -r " + input_file + " -w output/" + output_vector + ".pcap " + filter_out], shell=True, stdout=subprocess.PIPE)
    p.communicate()
    p.wait()
